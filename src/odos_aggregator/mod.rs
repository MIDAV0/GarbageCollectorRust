...
use crate::web3_client::web3_client::{Web3Client, GasMultiplier};
...
impl OdosAggregator {
    ...
    pub async fn swap(
        ...
        let quote = self.get_quote(&tokens_in, &tokens_out, amounts_in).await?;
        self.execute_swap(&tokens_in, quote).await?;
        Ok(())
    }
    ...
    async fn get_quote(
        ...
        if 
            tokens_in.iter().any(|t| Self::is_token_native(&t.address))
            && Self::is_token_native(&tokens_out[0].address) 
        {
